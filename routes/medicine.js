...
import { medicines, pharmacies } from "../data/store.js";
...
router.post("/", (req, res) => {
  const { name, category, quantity, expiry, donor, distanceKm, pickupLocation } = req.body;
  const newMedicine = {
...
router.post("/:id/request", (req, res) => {
  ...
  if (!medicine) return res.status(404).json({ error: "Medicine not found" });
  res.status(201).json({
    message: "تم إرسال طلبك، سيتم تسليم الدواء عبر الصيدلية الشريكة بعد فحصه والتأكد من صلاحيته.",
    medicine,
  });
});
